...
describe("SettingsView Codex section", () => {
  ...
  it("focuses the router credential input when opened with the credential target", async () => {
    cleanup();
    render(
      <SettingsView
        workspaceGroups={[]}
        ...
        onRemoveDictationModel={vi.fn()}
        initialSection="codex"
        initialTarget="auto-model-routing-credential"
      />,
    );

    const credentialInput = screen.getByLabelText("Router credential");
    await waitFor(() => {
      expect(document.activeElement).toBe(credentialInput);
    });
  });

  it("hides the router credential editor on mobile runtimes", async () => {
    cleanup();
    ...
    try {
      ...
      await waitFor(() => {
        expect(screen.getByText(/managed on the connected backend host/i)).toBeTruthy();
      });
      expect(screen.queryByLabelText("Router credential")).toBeNull();
      expect(screen.queryByRole("button", { name: "Save" })).toBeNull();
      expect(screen.queryByRole("button", { name: "Remove" })).toBeNull();
      expect(screen.getByRole("button", { name: "Manage credential" })).toBeTruthy();
    } finally {
